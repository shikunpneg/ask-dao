# -*- coding: utf-8 -*-
"""tools/engines/parallel_factory.py — 并行算力工厂：用满全部核去生成和扫描问题（R67）
...
def scan_two_sum(args):
    """反例驱动: A+B 覆盖, A=质数, B=多边形数(向量化标记)"""
    import numpy as np, bisect
    k, N = args
    N = min(N, 200000)
    primes = sorted(_primes(N))
    poly = []
    i = 1
    while True:
        v = i * ((k - 2) * i - (k - 4)) // 2
        ...
        poly.append(v)
        i += 1
    poly = [p for p in poly if p <= N]
    # 向量化: 标记所有 p+poly ≤ N 的数
    mark = np.zeros(N + 1, dtype=bool)
    for q in poly:
        kk = bisect.bisect_right(primes, N - q)
        if kk:
            mark[np.array(primes[:kk], dtype=np.int64) + q] = True
    exc = np.flatnonzero(~mark[4:]) + 4
...
def build_tasks(round_no):
    """进化式参数采样: 每轮 round_no 播种, 参数空间随轮扩大(不再固定重复)。"""
    rnd = random.Random(round_no * 131 + 17)
    growth = round_no
    tasks = []
    # 跨进制 b=3..(16+growth)
    for b in sorted(rnd.sample(range(3, 17 + growth), min(10, 14 + growth))):
        tasks.append(("palbase", b))
    # 两数和: 质数 + k 边形(k=3..(8+growth))
    for k in sorted(rnd.sample(range(3, 9 + growth), min(4, 6 + growth))):
        tasks.append(("two_sum", (k, 500000)))
    # 多边形模结构
    for k in sorted(rnd.sample(range(3, 13 + growth), min(4, 10 + growth))):
        tasks.append(("poly_spec", (k, 200000)))
    # 模迭代
    for c in sorted(rnd.sample(range(1, 6 + growth), min(4, 5 + growth))):
        tasks.append(("mod_iter", (c, 40)))
    # 组合禁子串(随轮生成更多)
    words = ["00", "11", "010", "101", "0000", "0010", "110", "011", "1010", "01010"]
    for w in sorted(rnd.sample(words[:8 + min(growth, 2)], min(5, 8 + min(growth, 2)))):
...
def main():
    t0 = time.time()
    # 读共享进化状态(与 all_domains_engine 共用轮次)
    stf = HERE / "out/demo/evolve_state.json"
    if stf.exists():
        try:
            state = json.loads(stf.read_text(encoding="utf-8"))
        except Exception:
            state = {"round": 0}
    else:
        state = {"round": 0}
    round_no = state["round"] + 1
    tasks = build_tasks(round_no)
    print("=" * 100)
    print(f"并行算力工厂 —— 用满 {os.cpu_count()} 核生成+扫描问题")
    print(f"  进化轮次: R{round_no} (参数空间随轮扩大)")
    print("=" * 100)
    print(f"  任务数: {len(tasks)} (跨进制/两数和/多边形/模迭代/组合)")
    print(f"  启动 {min(20, len(tasks))} 进程并行...")
    ...
    # 并入 manifest(去重: 跳过已见过的 statement)
    ...
    d["problems"] += [{**p, "id": f"PF_{start+i}"} for i, p in enumerate(new)]
    ...
    print(f"\n  并入 {len(new)} 条新问题 (去重 {len(all_probs)-len(new)}) -> manifest 总数 {d['count']}")
    print("  已存 out/demo/parallel_factory.json (本轮新序列)")
