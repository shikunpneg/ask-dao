# -*- coding: utf-8 -*-
"""tools/engines/all_domains_engine.py — 全领域并行引擎：数学/生物/物理/哲学/化学... 所有领域（R69）
...
def _math(param):
    ...
    # 标准 Collatz 链长序列(奇×3+1 偶÷2; 全收敛, 富含结构)
    def csteps(n):
        s = 0
        while n != 1:
            ...
            s += 1
        return s
    collatz = [csteps(n) for n in range(2, 40)]
    out.append({"domain": "数学", "statement": f"Collatz链长(n=2..39): {collatz[:10]}...",
                "evidence": {"seq": collatz}, "judge_route": "迭代枚举"})
...
def _phys(param):
    ...
    n, = param
    import itertools
    n = min(n, 8)  # 顶点数上限: 超过则枚举爆炸
    # 采样图, 算"存在阻挫(基态简并>1)"的比例, 得到随 n 的序列
    seq = []
    for m in range(3, n + 1):
        # 对 m 顶点完全图的子图采样(固定上限 512 张, 不随 2^|E| 爆炸)
        p2 = list(itertools.combinations(range(m), 2))
        frust = 0
        total = 0
        step = max(1, (2 ** len(p2)) // 512)  # 采样步进, 保证 ≈512 张
        for mask in range(0, 2 ** len(p2), step):  # 采样
...
def _info(param):
    ...
    n, = param
    n = min(n, 8)  # 上限: 枚举 2^n 个向量, n>8 爆炸
    import itertools
    # 枚举 n 位二元向量, 按汉明重量分组
    wt = [0] * (n + 1)
...
def _art(param):
    ...
    unpaired_seq = []  # 每个对称元的"未配对格点数"(小整数, 真正的结构签名)
    for s in syms:
        seen = set()
        unpaired = 0
        for cell in cells:
            ...
            seen.add(pr)
            unpaired += 1
        unpaired_seq.append(unpaired)
    # 未配对格点数序列: 恒等=n², 旋转90=ceil(n²/4)(n为偶数=n²/4), 翻转=ceil(n²/2)...
    orbit = sum(1 << u for u in unpaired_seq) // 8
    return [{"domain": "艺术", "statement": f"{n}x{n}二值纹样 D4 未配对格点数: {unpaired_seq}",
             "evidence": {"seq": unpaired_seq, "orbit": orbit}, "judge_route": "Burnside"}]
...
def build_tasks(state, round_no):
    """进化式参数采样: 每轮用 round_no 播种, 在逐轮扩大的参数空间里采样。

    state 记录已探索过的参数区间, 每轮向更大值扩展(进化), 避免固定参数空转。
    """
    rnd = random.Random(round_no)
    # 参数空间逐轮扩大: 第 r 轮把每个参数的上界推到 base + r
    growth = round_no
    tasks = []
    # 数学: (进制 b, k2, k3); b 范围随轮扩大
    for b in sorted(rnd.sample(range(3, 17 + growth), min(5, 15 + growth))):
        tasks.append(("数学", (b, 3, 4)))
    # 物理: n 随轮扩大
    for n in sorted(rnd.sample(range(4, 8 + growth), min(3, 4 + growth))):
        tasks.append(("物理", (n,)))
    # 生物: 种子随轮扩大
    for s in sorted(rnd.sample(range(1, 6 + growth), min(3, 5 + growth))):
        tasks.append(("生物", (s,)))
    # 化学
    for c in sorted(rnd.sample(range(4, 13 + growth), min(4, 9 + growth))):
        tasks.append(("化学", (c,)))
    # 心理
    for r in sorted(rnd.sample([x / 10 for x in range(1, 16 + growth)], min(3, 15 + growth))):
        tasks.append(("心理", (r,)))
    # 语言
    tasks.append(("语言", (round_no % 7,)))
    # 信息
    for n in sorted(rnd.sample(range(4, 9 + growth), min(2, 5 + growth))):
        tasks.append(("信息", (n,)))
    # 工程
    for r0 in sorted(rnd.sample([x / 10 for x in range(6, 10)], min(2, 4))):
        tasks.append(("工程", (r0,)))
    # 经济
    for a in sorted(rnd.sample([x / 10 for x in range(15, 30 + growth)], min(2, 15 + growth))):
        tasks.append(("经济", (a,)))
    # 音乐/艺术
    for n in sorted(rnd.sample(range(4, 9 + growth), min(2, 5 + growth))):
        tasks.append(("音乐", (n,)))
        tasks.append(("艺术", (n,)))
    # 伦理/哲学(定性)
    tasks.append(("伦理", (f"议题{round_no % 9}",)))
    tasks.append(("哲学", (f"概念{round_no % 7}",)))
    # 逻辑
    for n in sorted(rnd.sample(range(2, 5), min(2, 3))):
        tasks.append(("逻辑", (n,)))
    return tasks


def load_state():
    """读取进化状态(每轮参数游标), 不存在则初始化。"""
    f = HERE / "out/demo/evolve_state.json"
    if f.exists():
        try:
            return json.loads(f.read_text(encoding="utf-8"))
        except Exception:
            pass
    return {"round": 0, "explored": {}, "unseen_landmarks": []}
...
def main():
    ...
    # 去重: 跳过已见过的 statement 前缀(不再重复累积)
    seen = {p.get("statement", "")[:40] for p in d["problems"]}
    new = []
    for p in all_probs:
        key = p.get("statement", "")[:40]
        if key not in seen:
            seen.add(key)
            new.append(p)
    start = len(d["problems"])
    d["problems"] += [{**p, "id": f"AD_{start+i}"} for i, p in enumerate(new)]
    ...
    state["last_tasks"] = len(tasks)
    state["new_this_round"] = len(new)
    save_state(state)
    print(f"\n  并入 {len(new)} 条新问题 (去重 {len(all_probs)-len(new)} 重复) -> manifest 总数 {d['count']}")
    print("  已存 out/demo/all_domains.json (本轮新序列)")
