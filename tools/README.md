...
共 147 个脚本。按「**谁在运行它**」分组——只有被运行的才算活代码；只在历史日志里被提到的，不算。
...
| `fix_tools_layout_refs.py` | tools/ 分目录后，修 4 处兼容点。 |
| `fix_tools_layout_refs2.py` | tools/ 分目录后，修「查找/导入 tools 模块」的逻辑（不是改文案）。 |
...
| `reorganize_tools.py` | 整理 tools/：按「谁在运行它」分到子目录，并更新全部引用。 |
...
| 脚本 | 作用 |
|---|---|
| `fix_index_blurb.py` | 修 tools_index.py 的两处：(1) blurb 剥离前缀支持子目录 (2) 加手动入口白名单。 |
| `fix_index_header.py` | 给 tools/README.md 加一段「目录结构总览」（放在最前面，让人一眼看懂）。 |
| `fix_index_paths.py` | 修 tools_index.py：blurb 的取路径还在用根目录拼（文件已进子目录）。 |
