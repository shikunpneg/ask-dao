# -*- coding: utf-8 -*-
"""tools/research/corpus_to_problems.py — Track 1: 普通文本语料 → 日常疑问 → 科学问题
...
用法: python tools/research/corpus_to_problems.py [语料目录]
"""
import json
import re
import sys
...
def stage_b(q):
    """结构化: 该疑问能否映射到 grammar 框架 **且槽位能绑到具体载体**?

    **自纠错(第 17 次)**: 首版只问"有没有映射到框架", 而每个疑问类型我都预设了框架,
    于是报出 **100% 可结构化** —— 又一个"空通过"(见 R34 记录)。
    真正的判据是用户理论里的那句: "进行**针对领域的结构化**" ——
    结构化的意思是**槽位被具体载体填满**, 不是"句式对得上"。

    故本版要求: 疑问文本里必须能识别出**至少一个具体载体**
    (可枚举对象/类/映射), 否则只算"句式匹配", 不算结构化。
    """
    frames = Q2FRAME.get(q["type"], [])
    # 具体载体词表(可枚举对象/类/映射) —— 命中才叫"绑得上"
    carriers = {
        "数": ["质数", "素数", "合数", "偶数", "奇数", "平方", "立方", "三角数", "斐波那契",
               "整数", "自然数", "序列", "数列", "回文", "圆周率", "黄金", "对数", "指数"],
        "结构": ["图", "回路", "路径", "顶点", "边", "网络", "环", "群", "对称", "矩阵",
                 "集合", "函数", "方程", "概率", "分布", "周期", "频率"],
        "物理": ["速度", "光速", "能量", "质量", "温度", "熵", "力", "粒子", "波", "场",
                 "时间", "空间", "量子", "引力"],
        "生物": ["基因", "物种", "种群", "演化", "进化", "细胞", "遗传", "生命", "生态"],
        "认知": ["记忆", "注意", "知觉", "意识", "思维", "学习", "决策", "情绪"],
        "工程": ["算法", "计算", "复杂度", "效率", "系统", "设计", "优化"],
    }
    hit = []
    for dom, ws in carriers.items():
        got = [w for w in ws if w in q["text"]]
        if got:
            hit.append({"domain": dom, "hit": got})
    return {"frames": frames, "carriers": hit, "structured": bool(frames and hit)}
